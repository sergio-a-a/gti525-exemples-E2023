...
var serveRequest = function (request, response) {
  if (request.url.startsWith("/search?query=")) {
    ...
    response.write(
      "<html><body>Aucun resultat trouve pour l'expression: " +
        expr +
        "</body></html>"
    );
    response.end();
    ...
    response.statusCode = 200;
    response.write(
      "Merci d'avoir commande votre diplome! Ce dernier sera expedie a l'adresse suivante: " +
        expr
    );
    response.end();
    ...
    if (request.method == "POST") {
      request.on("data", function (blob) {
        let comment = decodeURIComponent(
          blob.toString().replaceAll("+", " ")
        ).replace("comment=", "");
